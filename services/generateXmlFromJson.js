...
<Naam>${c(data.locaties.at(-1).naam)}</Naam>
<Adres>${c(data.locaties.at(-1).adres)}</Adres>
<Postcode>${c(data.locaties.at(-1).postcode)}</Postcode>
<Plaats>${c(data.locaties.at(-1).plaats)}</Plaats>
<Land>${c(data.locaties.at(-1).land)}</Land>
<Voorgemeld>${c(fallbackOnwaar(data.locaties.at(-1).voorgemeld))}</Voorgemeld>
<Aankomst_verw></Aankomst_verw>
<Tijslot_van></Tijslot_van>
<Tijslot_tm></Tijslot_tm>
<Portbase_code>${c(data.locaties.at(-1).portbase_code)}</Portbase_code>
<bicsCode>${cleanBicsCode(data.locaties.at(-1).bicsCode)}</bicsCode>
</Locatie>
</Locaties>
<Financieel>
<Tarief>${formatBedrag(data.tarief)}</Tarief>
<BTW>0</BTW>
<ADR_toeslag_Chart>${formatBedrag(data.adrToeslagChart)}</ADR_toeslag_Chart>
<ADR_bedrag_Chart>${formatBedrag(data.adrBedragChart)}</ADR_bedrag_Chart>
<Botlek_Chart>${formatBedrag(data.botlekChart)}</Botlek_Chart>
...
<ExtraStop_Chart>${formatBedrag(data.extraStopChart)}</ExtraStop_Chart>
<GasMeten_Chart>${formatBedrag(data.gasMetenChart)}</GasMeten_Chart>
<Gen_Chart>${formatBedrag(data.genChart)}</Gen_Chart>
